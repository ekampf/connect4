...
class AwesomePlayer(Player):
    def get_move(self, state: GameState) -> int:
        ...
        # Then check if board is empty
        if self.board_is_empty(state.board):
            return columns // 2  # Start in the middle if board is empty

        # First check for winning moves
        col = self.check_winning_moves(state=state, in_a_row=4)
        if col is not None:
            return col

        # Then check for blocking moves
        col = self.check_for_blocking_moves(state=state, in_a_row=4)
        if col is not None:
            return col

        col = self.check_winning_moves(state=state, in_a_row=3)
        if col is not None:
            return col

        col = self.check_for_blocking_moves(state=state, in_a_row=3)
        if col is not None:
            return col

        return columns // 2  # Default to middle column if no other move found

    def check_winning_moves(self, state: GameState, in_a_row: int = 4):
        columns = len(state.board[0])
        rows = len(state.board)
        ...
        for col in range(columns):
            ...
            # Check if this move wins
            if self.check_winner(test_board, row, col, in_a_row):
                return col

        return None

    def check_for_blocking_moves(self, state: GameState, in_a_row: int = 4):
        columns = len(state.board[0])
        rows = len(state.board)
        opponent = 'O' if self.symbol == 'X' else 'X'
        for col in range(columns):
    ...
    def board_is_empty(self, board: List[List[str]]) -> bool:
        """Check if the board is empty"""
        return all(cell == ' ' for row in board for cell in row)

    def check_winner(self, board: List[List[str]], row: int, col: int, in_a_row: int = 4) -> bool:
        """Check if there's a winner on the test board"""
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        symbol = board[row][col]
        columns = len(board[0])
...
if __name__ == "__main__":
    ...
    strategies = [RandomPlayer, SimplePlayer, LousyPlayer, AwesomePlayer]
